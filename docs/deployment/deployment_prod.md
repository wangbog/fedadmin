...
3. **Prepare the configuration file**
   ...
   **⚠️ Important:** 
   ...
   - The application uses email for password recovery, password change notices, and new-user password setup links. Configure a reliable SMTP provider for production.
   - SMTP delivery failures are recorded and shown in `Federation Admin/System/Email Delivery`; they do not cancel the related account action. For newly created administrator users, administrators can copy the setup/reset link shown once after user creation.
   - Use `MAIL_SUPPRESS_SEND=True` only for temporary maintenance or controlled testing. Suppressed emails are also recorded in Email Delivery.
...
4. **Build and start the production container**
   ...
   **⚠️ Important:** The default timezone is `TZ=Asia/Shanghai` for China. Modify it in `docker-compose.prod.yml` if your server is located in another region, so the application logs use the correct local time.
   ...
   The first build may take a few minutes. After the container starts, Gunicorn runs automatically inside the `web` service.

5. **Initialize federation metadata certificate, database, and metadata files**

   Federation metadata signing requires `app/storage/private/federation/fed.crt` and `app/storage/private/federation/fed.key` before metadata is generated. For production, use your federation signing certificate whenever possible. Place your own `fed.crt` and `fed.key` in that directory, make sure they are readable by the container `fedadmin` user, and skip `flask init-certs`.
   ...
   docker compose -f docker-compose.prod.yml exec --user fedadmin web flask regenerate-metadata
   ```

   **⚠️ Important:** The generated password will be shown in the console, please keep it safe!
